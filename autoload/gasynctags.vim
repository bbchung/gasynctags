let s:dirty = 0
let s:busy = 0
fun gasynctags#update_if_need(init, force)
    if a:force == 1
        let s:dirty = 1
    endif
    ...
    if s:dirty == 1
        ...
        if a:init == 1
            if has('nvim') == 1
                call jobstart("global -u", {"in_io": "null", "out_io": "null", "err_io": "null", "exit_cb" : "gasynctags#on_updated"})
            else
                call job_start("global -u", {"in_io": "null", "out_io": "null", "err_io": "null", "exit_cb" : "gasynctags#on_updated"})
            endif
        else
            if has('nvim') == 1
...
fun gasynctags#on_updated(channel, msg)
    let s:busy = 0
    call gasynctags#update_if_need(0, 0)
endf
...
fun gasynctags#Enable()
    ...
    call gasynctags#update_if_need(1, 1)
    silent exe 'cs add ' . l:dir . '/GTAGS'

    silent! au! GasyncTagsEnable
    augroup GasyncTagsEnable
        au!
        au BufWritePost * call gasynctags#update_if_need(0, 1)
    augroup END
endf
